...
public partial class Events
{
    ...
    public void Publish<TEvt>(in TEvt evt)
    {
        ...
        if (_syncHandlers.TryGetValue(evtType, out var syncHandlersObj))
        {
            ...
            _logger.Debug?.Log($"Event published: {evtType.Name} Subscribers: {syncHandlers.Count}");
        ...
        if (_asyncHandlers.TryGetValue(evtType, out var asyncHandlersObj))
        {
            ...
            _logger.Debug?.Log($"Event published: {evtType.Name} Subscribers: {asyncHandlers.Count}");
    ...
    protected abstract void ScheduleAsyncHandlersRun<TEvt>(List<Func<TEvt, Task>> handlers, TEvt evt);
    ...
    private void ExecSyncHandlerLogging<TEvt>(Action<TEvt> handler, TEvt evt)
    {
        ...
        handler(evt);
        _logger.Debug?.Log($"Handler {handler.TryGetName()} ran for {evt} in {sw.ElapsedMilliseconds:F} ms");
    }
